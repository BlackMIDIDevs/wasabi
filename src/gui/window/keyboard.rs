        ...
        for (i, key) in key_view.iter_visible_keys() {
            if key.black {
                if let Some(color) = colors[i].map(map_color) {
                    ...
                    mesh.colored_vertex(
                        Pos2::new(map_x(key.left) + 2.0 * key_density, black_bottom - md_height),
                        darkened,
                    );

                    add_rect_triangles(&mut mesh);
                    mesh.colored_vertex(
                        Pos2::new(map_x(key.right) - 2.0 * key_density, top - black_key_overlap),
                        lightened,
                    );
                    mesh.colored_vertex(Pos2::new(map_x(key.right), top), darkened);
                    mesh.colored_vertex(
                        Pos2::new(map_x(key.right) - 2.0 * key_density, black_bottom - md_height),
                        lightened,
                    );
                    mesh.colored_vertex(
                        Pos2::new(map_x(key.right), black_bottom),
